...
- 未指定比例时，普通主体、头像、图标和方形素材默认使用 `1:1`；海报、卡牌、手机壁纸、横幅、封面等有明显画幅用途时，根据用途选择比例；用途和构图都无法判断且比例影响较大时再询问用户。
...
常用画幅建议：头像、图标和方形产品图使用 `1:1`；卡牌立绘和竖版海报使用 `2:3` 或 `9:16`；社交媒体竖图使用 `3:4` 或 `9:16`；横版海报和视频封面使用 `16:9`；商品横图使用 `3:2` 或 `16:9`。
