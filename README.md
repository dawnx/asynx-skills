...
未指定比例时，普通主体、头像、图标和方形素材默认使用 `1:1`。对于海报、卡牌、手机壁纸、横幅或封面，Agent 会根据用途选择合适画幅；如果用途和构图都无法判断，再向你确认。常用建议：卡牌立绘 `2:3` 或 `9:16`，社交媒体竖图 `3:4` 或 `9:16`，横版海报和视频封面 `16:9`，商品横图 `3:2` 或 `16:9`。
